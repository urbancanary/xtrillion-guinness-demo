...
# Import our enhanced bond analytics engine with profile support
from bond_master_hierarchy_enhanced import calculate_bond_master
from profile_config import get_calculation_flags, add_performance_metrics, filter_analytics_by_profile
...
def parse_and_calculate_bond():
    ...
    try:
        ...
        # PROFILE SYSTEM: Get calculation profile (NEW!)
        profile_param = request.args.get('profile')
        calc_flags = get_calculation_flags(profile_param)
        logger.info(f"🎯 Profile system: using profile '{profile_param or 'DEFAULT'}' with flags: {calc_flags}")
        ...
        # Call the enhanced master calculation function with profile support
        result = calculate_bond_master(
            ...
            calc_flags=calc_flags  # NEW: Profile-based field filtering
        )
        ...
        # Format response based on technical vs business request with PROFILE FILTERING
        if technical_response:
            # Full technical response with profile filtering
            all_analytics = {
                'yield': result.get('yield'),
                'duration': result.get('duration'),
                'spread': result.get('spread'),
                'accrued_interest': result.get('accrued_interest'),
                'clean_price': result.get('price'),
                'dirty_price': result.get('dirty_price'),
                'convexity': result.get('convexity'),
                'pvbp': result.get('pvbp'),
                'z_spread': result.get('z_spread'),
                'macaulay_duration': result.get('macaulay_duration'),
                'annual_duration': result.get('annual_duration'),
                'annual_macaulay_duration': result.get('annual_macaulay_duration'),
                'annual_yield': result.get('annual_yield')
            }
            
            # Apply profile filtering
            filtered_analytics = filter_analytics_by_profile(all_analytics, calc_flags)
            ...
            response = {
                ...
                'processing': {
                    ...
                    'calculation_engine': 'xtrillion_core_enhanced',
                    'confidence': 'high',
                    'route_used': result.get('route_used')
                },
                'summary': {
                    'enhanced_metrics_available': True,
                    'price_metrics': len([k for k in filtered_analytics if 'price' in k]),
                    'risk_metrics': len([k for k in filtered_analytics if any(x in k for x in ['duration', 'convexity', 'pvbp'])]),
                    'total_return_metrics': len(filtered_analytics)
                }
            }
        else:
            # Business-focused response with profile filtering
            all_analytics = {
                'accrued_interest': result.get('accrued_interest'),
                'annual_duration': result.get('annual_duration'),
                'annual_macaulay_duration': result.get('annual_macaulay_duration'),
        ...
        response = add_performance_metrics(response, profile_param, calc_flags)
        ...
        logger.info(f"🚀 Profile Performance: {response['processing']['performance_improvement']} improvement using '{response['processing']['profile_used']}' profile")
